...
imagor only serves a URL it can authorize. By default `generate` emits the `unsafe` form, which the server accepts only when run with `IMAGOR_UNSAFE`. For production, run imagor with `IMAGOR_SECRET` and sign the path yourself, passing `{ unsafe: false }` to get the bare signable path:
