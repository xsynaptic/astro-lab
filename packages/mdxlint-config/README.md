...
Without `--fix` it reports and exits non-zero on warnings or formatting drift (a quality gate). With `--fix` it rewrites changed files, and sets `editorialFixes` on the processor for [`@xsynaptic/remark-prose-rules`][remark-prose-rules] if a project uses it. `--root-path` defaults to the current working directory.
...
[remark-prose-rules]: https://github.com/xsynaptic/astro-lab/tree/main/packages/remark-prose-rules#readme
