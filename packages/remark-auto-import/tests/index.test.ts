...
describe('remarkAutoImport', () => {
	...
	test('preserves non-ASCII letters in a derived component name', () => {
		const tree = inject({ imports: ['./src/components/mdx/señor.astro'] }, 'post.mdx');

		expect(injectedImports(tree)[0]?.specifiers).toEqual(['Señor']);
	});

	test('rejects a filename that cannot become a valid component name', () => {
		expect(() => remarkAutoImport({ imports: ['./src/components/mdx/404.astro'] })).toThrow(
			/valid MDX component name/,
		);
	});
